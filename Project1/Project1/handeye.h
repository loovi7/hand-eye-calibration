...
void Tsai_HandEye(Mat Hcg, vector<Mat> Hgij, vector<Mat> Hcij)
{
	...
	for (int i = 0; i < nStatus; i++)
	{
		...
		Hcij[i](Rect(0, 0, 3, 3)).copyTo(Rcij);//将矩阵Hij[i]的（0,0）开始的3*3矩形部分复制给Rij
		...
		A.push_back(tempA);
		b.push_back(tempb);
		/*   Mat支持每一行插入（Mat::push_back），这一点与vector类似，将一行Mat插入到
		另外 一个Mat我们只需要保证两者的cols是相同的。如Mat mean;表示Mat的一行，现需要
		重复多行将mean压缩至Mat descripTop(mean.cols,0)当中，我们不需要特意指出descripTop矩阵
		中的rows的大小，即不需要事先知道rows的大小
		*/
	}
	...
	for (int i = 0; i < nStatus; i++)
	{
		...
		Hcij[i](Rect(3, 0, 1, 3)).copyTo(Tcij);//Rect函数，前两个参数当理解为坐标中的点（3，0），而非第三行第0列！
